...
import br.framework.gui.backend.management.AUTBKEProjectManament;
import br.framework.gui.eventos.AUTEventsButtonParallelProcess;
import br.framework.gui.eventos.AUTEventsTreeObject;
 ...
 *
 */
public class AUTProjectsManage{
	...
	public void configurarInicializacao() {
		...
		btAdicionarProj.addActionListener(new AUTEventsButtonParallelProcess() {			
			...
			public void autStartParallelProcess() {
				...
				conPjt.autGUIInsertProject(false,txtNomeProj.getText().trim(), txtDescricao.getText().trim());
				treeProjetos.setModel(new javax.swing.tree.DefaultTreeModel(conPjt.autGUITreeNodeFromProject(false)));
		...
		treeProjetos.addTreeSelectionListener(new AUTEventsTreeObject() {
			@Override
			public void autTreeObjectSelected(JTree e) {
	 ...
	 * 
	 */
	public AUTProjectsManage() {
		configurarInicializacao();
	}

	public AUTProjectsManage(javax.swing.JDialog form) {
		formPrincipal = form;
		configurarInicializacao();
	}
}
