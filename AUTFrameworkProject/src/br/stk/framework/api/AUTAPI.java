...
public class AUTAPI {
	/**
	 * Representa um processo que pode ser executado paralelamente a outros
	 * possui seções bem definidas:
	 * 
	 * Inicializao : Metódo - rotinasInicializacao() - Rotinas responsáveis pela preparação do ambiente;
	 * Execução : rotinasExecucao() - Rotinas relacionados ao processo principal que deverá ser execudo;
	 * finalizacao: rotinasFinalizacao() - Rotinas de finalização do sistema, liberação de recursos, geração de logs e etc...;
	 * 
	 * @author Softtek - QA
	 *
	 */
	public static abstract class AUTProcessoParalelo{
		public static class AUTControleProcesso{
			private String nomeProcesso = "";
			private static Integer id = 0;

			/**
			 * Retorna o nome do processo corrente
			 * 
			 * @return String - Nome do processo
			 * 
			 */
			public String retornarNomeProcesso() {
			 ...
			 */
			public static String retornarDescricao() {
				return String.format("INFO : PROCESSO ID : %s : CRIADO", id);
			}

			public AUTControleProcesso() {
				AUTGerenciadorLogs.registrarLog(retornarDescricao());
				id++;
			}

			public AUTControleProcesso(String nome) {
				nomeProcesso = nome;
				AUTGerenciadorLogs.registrarLog(retornarDescricao());
				id++;
			}
		...
		/**
		 * 
		 * Rotinas de inicialização do sistema
		 * 
		 */
		protected abstract void rotinasInicializacao();
		/**
		 * Rotinas de execução do processo como um todo, etapas principais que deverão
		 * ser concluídas
		 * 
		 */
		protected abstract void rotinasExecucao();
		/**
		 * Rotinas de finalização do sistema, processos de limpeza, liberação de recurso, gerações de logs e etc...
		 */
		protected abstract void rotinasFinalizacao();

		/**
		 * 
		 * ROTINAS DE MONITORAMENTO DO PROCESSO
		 * 
		 */
		protected abstract void rotinasMonitoramentoProcesso();


		public void pararExecucao() throws Exception {
		...
		public void executarProcesso() {
			/*
			rotinasInicializacao();
			rotinasExecucao();
			rotinasFinalizacao();
			 */

			javax.swing.SwingWorker swkExec = new javax.swing.SwingWorker() {
				@Override
				protected Object doInBackground(){

					System.out.println("**** PROCESSO PARALELO: ROTINAS DE INICIALIZACAO");
					rotinasInicializacao();
					System.out.println("**** PROCESSO PARALELO: ROTINAS DE EXECUCAO");

					try {
						System.out.println("INFOPROCPARALELO: EXECUCAO PROCESSO PRINCIPAL : INIT");

						rotinasExecucao();

						System.out.println("INFOPROCPARALELO: EXECUCAO PROCESSO PRINCIPAL : END");
					}
					catch(java.lang.Exception e) {
						System.out.println("INFOPROCPARALELOERROR: DURANTE EXECUCAO DA TAREFA PRINCIPAL");
						System.out.println(e.getMessage());
						e.printStackTrace();
					}

					System.out.println("**** PROCESSO PARALELO: ROTINAS DE MONITORAMENTO DO PROCESSO");
					this.cancel(true);
					javax.swing.SwingWorker swkMonitor = new javax.swing.SwingWorker() {
						@Override
						protected Object doInBackground() throws Exception {
							// TODO Auto-generated method stub
							try {
								System.out.println("INFOPROCPARALELO: ROTINA DE MONITORAMENTO : INIT");
								rotinasMonitoramentoProcesso();
								System.out.println("INFOPROCPARALELO: ROTINA DE MONITORAMENTO : END");
							}
							catch(java.lang.Exception e) {
								System.out.println("INFOPROCPARALELOERROR: EXECUCAO DA ROTINA DE MONITORAMENTO");
								System.out.println(e.getMessage());
								e.printStackTrace();
							}
					...
					System.out.println("**** PROCESSO PARALELO: ROTINAS DE FINALIZACAO");
					rotinasFinalizacao();
		...
		public void executarProcesso(boolean aguardaExecucao) {
	...
	public static class AUTProcessoExternoUtils{
		...
		public static class AUTDadosProcesso{
			 ...
			 * 
			 * @return String - Retorna a expressão regular de configuração do objeto
			 * 
			 */
		 ...
		 */
		/**
		 * @param linhaComando - Linha de comando
		 * @return java.lang.StringBuffer - Conteúdo enviado para o console de saída do processo
		 */
	 ...
	 *
	 */
	public static abstract class AUTGUIParallelProcess{
		public static class AUTControleProcesso{
			private String nomeProcesso = "";
			private static Integer id = 0;
			...
			public AUTControleProcesso() {
				AUTLogsManage.registrarLog(retornarDescricao());
				id++;
			}
	...
	public static class AUTJDBCConector{
		...
		public void configurarJDBCConector(String nomeClasseDriver,String stringConexao,String usuario,String senha) {
			AUTLogsManage.registrarLog("INFO : ALTERANDO : INICIALIZANDO CONECTOR JDBC");
			...
			AUTLogsManage.registrarLog("INFO : ALTERANDO : CONECTOR JDBC INICIALIZADO");
		}
		...
		public int executarSQL(boolean conectar,String cmdSql) {
			try {
				if(java.util.regex.Pattern.matches("?:i(SELECT)", cmdSql)) {
					...
					AUTLogsManage.registrarLog(String.format("INFO : COMANDO SQL NÃO RETORNA DADOS : %s",cmdSql));

					return -1;
				}
			} catch (SQLException e) {

				AUTLogsManage.registrarLog(e, String.format("ERRO : EXECUÇÃO DO PROCEDIMENTO SQL : %s",cmdSql));
		...
		public java.sql.Connection conectarBancoDeDados(){
			try {
				AUTLogsManage.registrarLog("INFO : CONECTANDO A BASE DE DADOS");
				java.lang.Class.forName(nomeClassJDBC).newInstance();				
				conTemp = java.sql.DriverManager.getConnection(strConexao, usr, pwd);
				AUTLogsManage.registrarLog("INFO : CONEXAO REALIZADA COM SUCESSO");

				return conTemp;
			}
			catch(java.lang.Exception e) {

				AUTLogsManage.registrarLog(e, String.format("ERRO : NÃO FOI POSSÍVEL CONECTAR A BASE DE DADOS COM STRINGCONEXAO: %s",strConexao));
		...
		public java.sql.Connection conectarBancoDeDadosAPI(){
			try {
				...
				AUTLogsManage.registrarLog("INFO : CONECTANDO A BASE DE DADOS VIA API");
				...
				AUTLogsManage.registrarLog(conTemp.getMetaData().getDatabaseProductName());
				AUTLogsManage.registrarLog(conTemp.getMetaData().getDatabaseProductName());
				AUTLogsManage.registrarLog("INFO : CONEXAO REALIZADA COM SUCESSO VIA API");
		...
		public void fecharConexao(java.sql.Connection conexao) {
			try {
				...
				AUTLogsManage.registrarLog(e,"ERRO: AO FECHAR A CONEXÃO COM O BANCO DE DADOS");
			}
		}
	...
	public static String gerarCaixaDialog(javax.swing.JDialog formulario){
		...
		AUTLogsManage.registrarLog(fileDialog.getSelectedFile());
	 ...
	 * @return java.util.List - Conteúdo do arquivo
	 * 
	 */
	public static java.util.List<String> carregarArquivo(String arquivo){
		...
		try {
			...
			AUTLogsManage.registrarLog(e, "ARQUIVO INFORMADO NAO ENCONTRADO");
		}
		...
		try {
			...
			while(linha != null) {
				conteudo.add(linha);
				AUTLogsManage.registrarLog(linha);
				linha = leitorArq.readLine();
			}
		} catch (IOException e) {
			AUTLogsManage.registrarLog(e,"ERRO NA LEITURA DO ARQUIVO");
		}
