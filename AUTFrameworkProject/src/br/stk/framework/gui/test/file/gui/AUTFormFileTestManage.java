package br.stk.framework.gui.test.file.gui;
...
import br.stk.framework.gui.logs.*;
public class AUTFormFileTestManage{
	...
	public void analisarConteudo(Object conteudo) {
		AUTLogsManage.registrarLog("VALIDANDO EXPRESSAO REGULAR");
		AUTLogsManage.registrarLog(conteudo);
		for(java.lang.Character caractere : conteudo.toString().toCharArray()) {
			AUTLogsManage.registrarLog(String.format("Caractere: %s Especial : %s Alfabeto : %s Digito : %s", 
		...
		AUTLogsManage.registrarLog(conteudo.toString().getClass().getName());
	}
	...
	private void configurarGUI() {
			...
			public void keyReleased(KeyEvent e) {				
				if(e.getKeyCode() >= 37 && e.getKeyCode() <= 40) {
					if(conteudoArquivo.getSelectedText() !=null) {
						AUTLogsManage.registrarLog(retornarConteudoCaixaTexto(conteudoArquivo));
		...
		botaoAdicCampo.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(java.awt.event.ActionEvent e) {
				...
				tabelaItensSalvos.setValueAt(colunaCorrenteCapturaSessao, posicaoTabelaCorrenteSessaoEdit, 0);
				AUTLogsManage.registrarLog(colunaCorrenteCapturaSessao);
			}
		});
		...
		tabelaItensSalvos.addMouseListener(new java.awt.event.MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				AUTLogsManage.registrarLog("COLUNA SELECIONADA TABELA: ");
				AUTLogsManage.registrarLog(colunaTabelaSessaoSelecionada);
				
				AUTLogsManage.registrarLog("LINHA SELECIONADA NA TABELA: ");
	...
	public void executarAplicacao() {
		try {
			AUTLogsManage.registrarLog("*** INICIALIZANDO APLICATIVO DE CONFIGURACAO DE ARQUIVOS");
			configurarGUI();
			AUTLogsManage.registrarLog("*** FINALIZANDO APLICATIVO DE CONFIGURACAO DE ARQUIVOS");
		}
		catch(java.lang.Exception e) {
			AUTLogsManage.registrarLog(e, "ERRO NA CONSTRUÇÃO DA INTERFACE GRAFICA DA APLICAÇÃO");
		}		
	}
	
	public AUTFormFileTestManage() {
		super();
		executarAplicacao();
	}
	
	
	public AUTFormFileTestManage(javax.swing.JDialog formularioPrincipal) {
		super();
